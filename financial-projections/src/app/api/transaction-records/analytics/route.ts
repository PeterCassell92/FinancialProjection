...
import { TransactionRecordWhereClause } from '@/types';
...
export const { GET } = defineRoute({
  ...
  action: async ({ queryParams }) => {
    try {
      const parsedParams = {
        bankAccountId: queryParams?.bankAccountId,
        startDate: queryParams?.startDate || undefined,
        endDate: queryParams?.endDate || undefined,
        responseFormat: queryParams?.responseFormat || 'json',
      };

      const validation = analyticsQuerySchema.safeParse(parsedParams);
  ...
  responses: {
    200: { description: 'Transaction analytics retrieved successfully' },

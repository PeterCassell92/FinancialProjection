...
Get a single projection event by ID, including all event details and relationships.
...
**Note:** `recurringRuleId` will be null for one-off events, or contain the UUID of the parent recurring rule if this event was generated from a recurring event rule.

### PUT /api/projection-events/[id]

Update a projection event. This endpoint performs partial updates - only the fields provided in the request body will be updated.
...
- `value` (optional): Event amount (positive number)
- `type` (optional): `EXPENSE` or `INCOMING`
- `certainty` (optional): `UNLIKELY`, `POSSIBLE`, `LIKELY`, or `CERTAIN`
- `payTo` (optional): Payee for expenses
- `paidBy` (optional): Payer for incoming payments
- `date` (optional): Event date (YYYY-MM-DD)
- `bankAccountId` (optional): Bank account UUID
- `decisionPathId` (optional): Decision path UUID (null to remove)
- `recurringRuleId` (optional): Parent recurring rule UUID - **preserves the link** to the recurring rule that generated this event. This is important for maintaining parent-child relationships when editing individual events from a recurring series.
...
**Important Notes:**
- When editing an event that was created by a recurring rule, include the `recurringRuleId` to maintain the parent-child relationship
- This allows bulk operations (like deleting all events from a rule) to still work correctly
- Balances are automatically recalculated for the next 6 months after the event date

### DELETE /api/projection-events/[id]

Delete a single projection event by ID.
...
**Important Notes:**
- This deletes individual events, even if they were generated by a recurring rule
- The event's link to its parent recurring rule (if any) is preserved in the database until deletion
- To delete **all events** from a recurring rule, delete the rule itself via `DELETE /api/recurring-event-rules/[id]` (cascade delete)
- Balances are automatically recalculated for the next 6 months after the deleted event's date
